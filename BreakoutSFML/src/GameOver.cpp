...
// the HTTP response body is a single line of plain text in the form:
// name1,score1;name2,score2;name3,score3;...name10,score10
...
void GameOver::saveScore(Game* game) {
    tgui::EditBox::Ptr name = game->getOverGui().get<tgui::EditBox>("playername");

    std::stringstream url;
    url << "scores/scores.aspx?apikey=11&action=write"
        << "&name=" << name->getText().toAnsiString()
        << "&score=" << game->getScore();
    sf::Http http("host.domain");
