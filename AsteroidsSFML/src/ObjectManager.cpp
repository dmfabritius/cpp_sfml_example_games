...
void ObjectManager::add(GameObject* gameObject, std::string name) {
    if (name != "") {
        gameObjects[name] = gameObject;
    } else {
        std::stringstream unique;
        unique << 'N' << ++uniqueID;
        gameObjects[unique.str()] = gameObject;
    }
}
...
void ObjectManager::remove(std::string name) {
    ...
    if (obj != gameObjects.end()) {
        ...
        gameObjects.erase(obj); // remove it from the collection of game objects
    }
}

void ObjectManager::remove(GameObject* gameObject) {
...
void ObjectManager::update() {
    ...
    while (obj != gameObjects.end()) {
        if (obj->second->isDestroyed()) {
            delete obj->second;           // deallocate the game object from the heap
            obj = gameObjects.erase(obj); // remove it from the collection of game objects
        } else {
