...
sf::Sound* SoundFileCache::loadSound(const std::string & filename) {
    ...
    if (buffer->loadFromFile(filename)) {
        ...
        delete buffer;
        delete sound;
        throw std::runtime_error(filename + " was not found in SoundFileCache::getSound");
    }
    return sound;
}
...
sf::Music* SoundFileCache::loadSong(const std::string & filename) {
    ...
    if (music->openFromFile(filename)) {
        ...
        delete music;
        throw std::runtime_error(filename + " was not found in call to SoundFileCache::getSong");
    }
    return music;
}
